...
router.post('/sign-in', async (req, res, next) => {
    try {
        ...
        // Access Token 생성
        const accessToken = jwt.sign({ accountId: user.accountId }, process.env.ACCESS_TOKEN_SECRET_KEY, { expiresIn: '1h' });

        // Refresh Token 생성
        const refreshToken = jwt.sign({ accountId: user.accountId }, process.env.REFRESH_TOKEN_SECRET_KEY, { expiresIn: '1d' });
